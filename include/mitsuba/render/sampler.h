 ...
 *        \ref seed() to initialize a new sequence with the specific seed offset.
 *     2. The first pixel sample can now be computed, after which \ref advance() needs to be
 ...
 *        of (pseudo-) random numbers using the \ref next_1d() and \ref next_2d() functions
 ...
 *        This can be achieved by calling \ref set_samples_per_wavefront().
 *     2. Then the rendering algorithm should seed the sampler and set the appropriate wavefront
 *        size by calling \ref seed(). A different seed value, based on the \ref base_seed and
 *        the seed offset, will be used for every sample (of every pixel) in the wavefront.
 *     3. \ref advance() can be used to advance to the next sample in the sequence. When
 *        multiple samples per pixel are rendered in parallel in the wavefront, the appropriate
 *        sample index offset will be applied so as to not compute the same sample twice.
 *     4. As in the scalar approach, the rendering algorithm can request batches of (pseudo-)
 *        random numbers using the \ref next_1d() and \ref next_2d() functions.
 *
 */
...
protected:
    ...
    UInt32 compute_per_sequence_seed(uint64_t seed_offset) const;
...
protected:
    ...
    /// Array of offsets to compute current sample index in wavefront modes

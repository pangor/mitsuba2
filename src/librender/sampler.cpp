...
MTS_VARIANT Sampler<Float, Spectrum>::Sampler(const Properties &props) {
    ...
    m_wavefront_sample_offsets = UInt32(0);
}
...
MTS_VARIANT void Sampler<Float, Spectrum>::seed(uint64_t /*seed_offset*/,
    ...
    m_wavefront_sample_offsets =
        arange<UInt32>(wavefront_size) % UInt32(m_samples_per_wavefront);
...
MTS_VARIANT uint32_t Sampler<Float, Spectrum>::wavefront_size() const {
    return enoki::slices(m_wavefront_sample_offsets);
}
...
Sampler<Float, Spectrum>::compute_per_sequence_seed(uint64_t seed_offset) const {
    UInt32 indices = arange<UInt32>(wavefront_size());
...
Sampler<Float, Spectrum>::current_sample_index() const {
    return m_sample_index * m_samples_per_wavefront + m_wavefront_sample_offsets;
}

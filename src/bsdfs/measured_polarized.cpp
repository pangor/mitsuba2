...
public:
    ...
    MeasuredPolarized(const Properties &props) : Base(props) {
        ...
        if (!(theta_h.shape.size() == 1 &&
              ...
              pbrdf.shape.size() == 6 &&
              pbrdf.shape[0] == phi_d.shape[0] &&
              pbrdf.shape[1] == theta_d.shape[0] &&
              pbrdf.shape[2] == theta_h.shape[0] &&
              pbrdf.shape[3] == wavelengths.shape[0] &&
              pbrdf.shape[4] == 4 &&
              pbrdf.shape[5] == 4)) {
    ...
    Spectrum eval(const BSDFContext &ctx, const SurfaceInteraction3f &si,
        ...
        if constexpr (is_spectral_v<Spectrum>) {
            if constexpr (is_polarized_v<Spectrum>) {
                ...
                // Make sure pBRDF intensity is non-negative
                ...
                if (m_lambda == -1.f) {
                    ...
                    value3 = Spectrum(m_interpolator.eval(Point2f(0.f, 0.f), params, active));
                }
...
private:
    ...
    Vector3 rotate_vector(const Vector3 &v, const Vector3 &axis_, Value angle) const {
        ...
        auto [cos_angle, sin_angle] = sincos(angle);
